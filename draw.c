...
void onmessage(int fd, const unsigned char *msg, uint64_t size, int type)
{
     ...
     * again.
     */
    struct wsmsg *wm = (struct wsmsg *)malloc (sizeof (struct wsmsg));
    wm->fd   = fd;
    wm->type = type;
    wm->obj  = json_tokener_parse((char *)msg);
    pthread_mutex_lock( &mutex );
    writeFifo (&tfifo, (void *)wm);
...
void *on_thread()
{
	...
	struct wsmsg *wm = (struct wsmsg *)data;
	json_object *obj = wm->obj;
	int   fd = wm->fd;
	int type = wm->type;
        int width, height;
